// Code taken and modified from miniunzip implementation for zlib version 1.2.7
...
int do_extract_currentfile( unzFile uf, const int* popt_extract_without_path,
                            int* popt_overwrite, const char* password, vector<AbstractImage<pixel> > & images)
{
...
int do_extract( unzFile uf, int opt_extract_without_path, int opt_overwrite, const char* password, vector< AbstractImage<pixel> > & images )
{
    uLong i;
...
void ImagePreprocessor::_CBZ(rawData data) {
    ...
    do_extract(uf, 1, 1, NULL, this->images);
